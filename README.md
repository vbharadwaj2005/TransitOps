...
Unique Identification: Reject duplicate alphanumeric vehicle registration_number keys and driver license_number strings.
...
Capacity Constraints: Trip cargo weight cannot exceed the selected vehicle's max_load_capacity.
Atomic Trip Transitions:
...
Maintenance Cycles: Opening a maintenance ticket routes a vehicle to In Shop. Closing the log records the expense and returns the vehicle to Available.
📈 Key Formulas Used
Fuel Efficiency: 
Fuel Efficiency
=
...
∑
Other Expenses
Total Operational Cost=∑Fuel Expenses+∑Maintenance Expenses+∑Other Expenses
...
Vehicle ROI= 
Acquisition Cost
∑Trip Revenue−(Total Maintenance Cost+Total Fuel Cost)
​
...
Seed the database (creates tables & local SQLite database file transitops.db with sandbox accounts):
bash
