...
def get_trips() -> tuple[Any, int]:
    ...
    if search:
        query = query.where(db.or_(Trip.source.ilike(f'%{search}%'), Trip.destination.ilike(f'%{search}%')))
...
@jwt_required()
def get_trip(trip_id: int) -> tuple[Any, int]:
...
@jwt_required()
@role_required(['Fleet Manager', 'Driver'])
def dispatch_trip(trip_id: int) -> tuple[Any, int]:
    trip = db.get_or_404(Trip, trip_id)
...
@jwt_required()
@role_required(['Fleet Manager', 'Driver'])
def complete_trip(trip_id: int) -> tuple[Any, int]:
    trip = db.get_or_404(Trip, trip_id)
...
@jwt_required()
@role_required(['Fleet Manager', 'Driver'])
def cancel_trip(trip_id: int) -> tuple[Any, int]:
    trip = db.get_or_404(Trip, trip_id)
