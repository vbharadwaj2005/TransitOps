...
@jwt_required()
def get_vehicles() -> tuple[Any, int]:
    ...
    if search:
        query = query.where(db.or_(Vehicle.registration_number.ilike(f'%{search}%'), Vehicle.model.ilike(f'%{search}%')))
    if v_type:
        query = query.where(Vehicle.type == v_type)
    if status:
...
@jwt_required()
def get_vehicle(vehicle_id: int) -> tuple[Any, int]:
...
def create_vehicle() -> tuple[Any, int]:
    ...
    existing = db.session.execute(db.select(Vehicle).where(Vehicle.registration_number == reg_num)).scalar()
    if existing:
...
@jwt_required()
@role_required(['Fleet Manager'])
def update_vehicle(vehicle_id: int) -> tuple[Any, int]:
    vehicle = db.get_or_404(Vehicle, vehicle_id)
    data = request.get_json() or {}
    ...
    if reg_num:
        reg_num = reg_num.strip().upper()
        existing = db.session.execute(db.select(Vehicle).where(Vehicle.registration_number == reg_num, Vehicle.id != vehicle_id)).scalar()
        if existing:
...
@jwt_required()
@role_required(['Fleet Manager'])
def delete_vehicle(vehicle_id: int) -> tuple[Any, int]:
    vehicle = db.get_or_404(Vehicle, vehicle_id)
