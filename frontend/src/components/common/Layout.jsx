...
const Layout = ({ children }) => {
  ...
  return (
    <div className="flex h-screen w-screen overflow-hidden bg-[#070a13] text-[#f8fafc]">
      {/* Sidebar */}
      <aside className="w-64 flex-shrink-0 flex flex-col justify-between border-r border-slate-800 bg-[#0c101e] px-4 py-6">
        <div>
          {/* Logo */}
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium transition-all duration-200 ${
                    isActive
                      ? 'bg-gradient-to-r from-indigo-600/25 to-indigo-600/5 text-indigo-400 border-l-2 border-indigo-500'
        ...
        <div className="space-y-4 pt-4 border-t border-slate-800/80">
          ...
          <button
            ...
            className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-rose-400 hover:bg-rose-950/20 transition-all duration-200"
          >
            <LogOut size={20} />
            Sign Out
          </button>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Top Navbar */}
        <header className="h-16 flex items-center justify-between px-8 border-b border-slate-800 bg-[#090d19]">
          <div className="flex items-center gap-2">
            <span className="text-xs font-semibold px-2.5 py-1 rounded-full bg-indigo-950/50 text-indigo-400 border border-indigo-800/30">
              System Live
            </span>
          </div>
        ...
        {/* Page Content */}
        <main className="flex-1 overflow-y-auto p-8 bg-[#070a13]">
          {children}
        </main>
      </div>
    </div>
