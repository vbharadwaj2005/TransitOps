...
import { Truck, Users, Wrench, TrendingUp, RotateCcw } from 'lucide-react';
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <button onClick={fetchData} className="flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 transition-colors shadow-sm cursor-pointer">
          <RotateCcw size={16} /> Refresh Stats
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="glass-card rounded-2xl p-6 relative overflow-hidden group hover:translate-y-[-4px] transition-all duration-300">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="p-2 rounded-lg bg-indigo-50 text-indigo-600"><TrendingUp size={20} /></div>
          </div>
          <div className="flex items-baseline gap-2"><span className="text-3xl font-extrabold text-slate-900">{kpis.fleet_utilization}%</span></div>
          <div className="mt-4 w-full bg-slate-100 rounded-full h-2">
            <div className="bg-indigo-600 h-2 rounded-full transition-all duration-1000" style={{ width: `${Math.min(kpis.fleet_utilization, 100)}%` }} />
          </div>
        </div>
        ...
        <div className="glass-card rounded-2xl p-6 relative overflow-hidden group hover:translate-y-[-4px] transition-all duration-300">
          ...
          <div className="flex items-baseline gap-2"><span className="text-3xl font-extrabold text-slate-900">{kpis.active_vehicles}</span><span className="text-xs text-slate-500">on trip</span></div>
        ...
        <div className="glass-card rounded-2xl p-6 relative overflow-hidden group hover:translate-y-[-4px] transition-all duration-300">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="p-2 rounded-lg bg-emerald-50 text-emerald-600"><Users size={20} /></div>
          </div>
          <div className="flex items-baseline gap-2"><span className="text-3xl font-extrabold text-slate-900">{kpis.drivers_on_duty}</span><span className="text-xs text-slate-500">active</span></div>
        ...
        <div className="glass-card rounded-2xl p-6 relative overflow-hidden group hover:translate-y-[-4px] transition-all duration-300">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="p-2 rounded-lg bg-amber-50 text-amber-600"><Wrench size={20} /></div>
          </div>
          <div className="flex items-baseline gap-2"><span className="text-3xl font-extrabold text-slate-900">{kpis.in_shop_vehicles}</span><span className="text-xs text-slate-500">repairing</span></div>
          ...
          <div className="glass-card rounded-2xl p-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
              <div>
                ...
                <select value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} className="block w-full rounded-lg border border-slate-300 bg-white py-2 px-3 text-sm text-slate-800 outline-none focus:border-indigo-500">
              ...
              <div>
                ...
                <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="block w-full rounded-lg border border-slate-300 bg-white py-2 px-3 text-sm text-slate-800 outline-none focus:border-indigo-500">
            ...
            <div className="flex items-center justify-between border-t border-slate-200 pt-4">
              <span className="text-xs font-semibold text-slate-500">Showing {filteredVehicles.length} of {vehicles.length} assets</span>
              {(typeFilter || statusFilter) && (
                <button onClick={() => { setTypeFilter(''); setStatusFilter(''); }} className="text-xs font-semibold text-indigo-600 hover:text-indigo-700 transition-colors cursor-pointer">Clear Filters</button>
              )}
            </div>
          </div>
               ...
               <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredVehicles.length === 0 ? (
                    <tr><td colSpan={6} className="py-8 text-center text-slate-400">No vehicles matching the filters found.</td></tr>
                  ) : (
