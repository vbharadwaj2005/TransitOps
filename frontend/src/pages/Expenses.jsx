  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold tracking-tight text-slate-900">Expenses Ledger</h2>
          <p className="text-slate-500 mt-1">Audit vehicle operational expenditures, fuel receipts, and manual bills.</p>
        </div>
        <div className="flex flex-wrap gap-2">
          {/* Log Fuel */}
          <button
            onClick={openFuelModal}
            className="flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 transition-colors cursor-pointer"
          >
            <Fuel size={16} />
            Log Fuel
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="glass-card rounded-2xl p-6 flex items-center justify-between">
          <div>
            <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-1">Total Expenditures</p>
        ...
        <div className="md:col-span-2 glass-card rounded-2xl p-6 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <span className="text-sm font-semibold text-slate-700">Filter Ledger</span>
          </div>
          <select
            value={typeFilter}
            onChange={(e) => setTypeFilter(e.target.value)}
            className="rounded-lg border border-slate-350 bg-white py-2 px-4 text-sm text-slate-800 outline-none focus:border-indigo-500 w-48 focus:ring-1 focus:ring-indigo-500"
          >
                ...
                {filteredExpenses.map((exp) => (
                  <tr key={exp.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="py-4">
                      {exp.vehicle_reg ? (
                        <div>
                          <p className="font-semibold text-slate-900">{exp.vehicle_reg}</p>
                          <p className="text-[10px] text-slate-500">{exp.vehicle_model}</p>
                        </div>
                      ) : (
                        <span className="text-slate-400 italic text-xs">General Fleet</span>
                      )}
                    </td>
                    <td className="py-4 text-slate-700 max-w-sm truncate" title={exp.description}>
                      {exp.description}
                    </td>
