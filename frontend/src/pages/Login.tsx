    ...
    <div className="flex min-h-screen items-center justify-center bg-slate-100 px-4 py-12 relative overflow-hidden">
      <div className="w-full max-w-5xl flex flex-col md:flex-row items-stretch rounded-2xl overflow-hidden shadow-xl border border-slate-200 bg-white">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              ...
              <div className="relative">
                ...
                <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="manager@transitops.com" className="block w-full rounded-xl border border-slate-300 bg-white py-3 pl-11 pr-4 text-sm text-slate-950 placeholder-slate-400 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/30 outline-none transition-all" required />
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                ...
                <input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="block w-full rounded-xl border border-slate-300 bg-white py-3 pl-11 pr-4 text-sm text-slate-950 placeholder-slate-400 focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/30 outline-none transition-all" required />
              </div>
            </div>
            <button type="submit" disabled={isSubmitting} className="w-full rounded-xl bg-indigo-600 hover:bg-indigo-500 py-3 px-4 text-sm font-semibold text-white shadow-md hover:shadow-indigo-600/10 transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer">
              {isSubmitting ? 'Signing in...' : 'Sign In to TransitOps'}
            </button>
          </form>
          <p className="text-xs text-slate-500 text-center mt-4">
            Don't have an account?{' '}
            <Link to="/register" className="text-indigo-600 hover:text-indigo-500 font-semibold">Create one</Link>
          </p>
        </div>
        ...
        <div className="w-full md:w-1/2 p-8 md:p-12 border-t md:border-t-0 md:border-l border-slate-200 bg-slate-50 flex flex-col justify-center">
          ...
          <p className="text-xs text-slate-500 mb-6 leading-relaxed">Role-Based Access Control is enforced. Click on any sandbox role below to auto-fill its credentials and explore its specific capabilities.</p>
          <div className="space-y-3">
            {credentials.map((credential) => (
              <button key={credential.email} onClick={() => fillCredentials(credential.email)} className="w-full text-left p-4 rounded-xl border border-slate-200 bg-white hover:bg-slate-50 hover:border-slate-300 transition-all group cursor-pointer">
                <div className="flex items-center justify-between mb-1">
                  <span className="text-xs font-bold text-indigo-600 group-hover:text-indigo-500 transition-colors">{credential.role}</span>
                  <span className="text-[10px] px-2 py-0.5 rounded bg-slate-100 text-slate-500 group-hover:text-slate-700 transition-colors">Auto-Fill</span>
                </div>
